  ...
  def visit(node: AstNode, arg: Unit): Unit = {
    ...
    node match {
      case AAssignStmt(id1: AIdentifier, alloc: AAlloc, _) =>
        // X = alloc P: [[X]] = ^[[alloc-i]]
        unify(identifierToTerm(id1), PointerRef(allocToTerm(alloc)))

      case AAssignStmt(id1: AIdentifier, AVarRef(id2: AIdentifier, _), _) =>
        // X1 = &X2: [[X1]] = ^[[X2]]
        unify(identifierToTerm(id1), PointerRef(identifierToTerm(id2)))

      case AAssignStmt(id1: AIdentifier, id2: AIdentifier, _) =>
        // X1 = X2: [[X1]] = [[X2]]
        unify(identifierToTerm(id1), identifierToTerm(id2))

      case AAssignStmt(id1: AIdentifier, AUnaryOp(DerefOp, id2: AIdentifier, _), _) =>
        // X1 = *X2: [[X2]] = ^a && [[X1]] = a
        val freshVar = FreshVariable()
        unify(identifierToTerm(id2), PointerRef(freshVar))
        unify(identifierToTerm(id1), freshVar)

      case AAssignStmt(ADerefWrite(id1: AIdentifier, _), id2: AIdentifier, _) =>
        // *X1 = X2: [[X1]] = ^a && [[X2]] = a
        val freshVar = FreshVariable()
        unify(identifierToTerm(id1), PointerRef(freshVar))
        unify(identifierToTerm(id2), freshVar)
